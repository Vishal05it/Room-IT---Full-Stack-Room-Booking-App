...
export const slotParser = (slotArr: string[]) => {
  let finalSLots = [];
  let finalIdx = 0;
  for (let refSlots of referenceSlots) {
    let found = false;
    //console.log(`Ref slot currently = ${refSlots.value}`);
    for (let freeSlot of slotArr) {
      if (freeSlot == refSlots.value) {
        // console.log(
        //   `Free slot = ${freeSlot} & refSlot value - ${refSlots.value}`,
        // );
        found = true;
        break;
      }
